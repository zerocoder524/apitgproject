...
async def weather(message: types.Message):
    args = message.text.split(maxsplit=1)  # Разбиваем текст сообщения на части
    if len(args) < 2:
        await message.reply("Пожалуйста, укажите город.")
        return

    city = args[1].strip()  # Берем второй элемент (город) и убираем лишние пробелы
...
async def start(message: Message):
    await message.answer('Привет! Отправь команду /weather <город>, чтобы получить прогноз погоды')
