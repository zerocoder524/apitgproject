...
async def finances_expenses3(message: Message, state: FSMContext):
    ...
    try:
        ...
        logging.error(f"Ошибка при обновлении данных для пользователя {telegram_id}: {e}")
...
async def view_users(message: Message):
    ...
    # Создаем таблицу
    table = PrettyTable()
    table.fieldnames = ["ID", "Telegram ID", "Имя", "Категория 1", "Расходы 1", "Категория 2", "Расходы 2", "Категория 3", "Расходы 3"]

    for user in users:
        table.add_row(user)

    await message.answer(f"Содержимое базы данных:\n{table}")
